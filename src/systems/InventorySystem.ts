...
export class InventorySystem {
		...
		if (!this.mouse.pressed && this.draggedItem) {
			if (isChip(this.draggedItem)) {
				...
				if (targetSlot !== -1 && targetSlot < 5) {
					...
					const draggedChip = this.draggedItem;
					...
					if (this.dragStartSlot !== null) inventory.chips[this.dragStartSlot] = this.draggedItem;
				}
			} else if (isModifier(this.draggedItem)) {
				...
				if (
					...
					inventory.modifiers[targetSlot] = this.draggedItem;
					if (this.dragStartSlot !== null) inventory.modifiers[this.dragStartSlot] = oldMod;
				} else {
					if (this.dragStartSlot !== null) inventory.modifiers[this.dragStartSlot] = this.draggedItem;
				}
			}
	...
	private getChipSlotAt(mouseX: number, mouseY: number): number {
		...
		for (let i = 0; i < 5; i++) {
			...
			if (pir({ x: mouseX, y: mouseY }, rect)) return i;
		}
		return -1;
	}
